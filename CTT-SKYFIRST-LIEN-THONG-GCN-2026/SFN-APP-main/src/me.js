...
export async function meRoute(request,env,url){
  ...
  if(p==="/api/me/internal-requests"&&request.method==="POST"){
    ...
    const allowed=["Cập nhật thông tin nhân sự","Điều chuyển vị trí/đơn vị","Tạm ngừng hoạt động","Thôi nhiệm vụ/rút khỏi SFN","Đề nghị xác nhận quá trình tham gia","Đề nghị GCN/GXN","Bàn giao nhiệm vụ","Khác"];
    if(!allowed.includes(body.request_type)||String(body.content||"").trim().length<3) return json({error:"INVALID_INPUT"},400);
    const code=await nextCode(env,"SFN-YC");
