...
export async function publicRoute(request,env,url){
  ...
  if(p==="/api/config"&&request.method==="GET"){
    ...
    return json({
      app_name:await getSetting(env,"app_name","Sky First Network"),
      ...
      hero_text:await getSetting(env,"hero_text","Một cổng chung cho Thành viên, Core Team, Tình nguyện viên, Học sinh/Học viên, lớp học, hoạt động, hồ sơ, GCN/GXN và quản trị Sky First Network."),
      ...
      header_show_account:await getSetting(env,"header_show_account",true),
      ...
      footer_support_email:await getSetting(env,"footer_support_email","hotro@skyfirst.io.v"),
      ...
      portal_main_url:await getSetting(env,"portal_main_url","https://www.skyfirst.io.vn"),
  ...
  if(p==="/api/public/news"&&request.method==="GET"){
    return json(await safePublicList(env,"SELECT id,title,slug,body,published_at,tags_json,cover_file_id FROM news WHERE status='published' ORDER BY COALESCE(published_at,created_at) DESC LIMIT 100"));
  }
  if(p==="/api/public/classes"&&request.method==="GET"){
    return json(await safePublicList(env,"SELECT id,unit_code,title,level,status,schedule_json,capacity,data_json FROM classes ORDER BY created_at DESC"));
  }
  if(p==="/api/public/events"&&request.method==="GET"){
    return json(await safePublicList(env,"SELECT id,unit_code,title,start_at,end_at,status,capacity,data_json FROM events ORDER BY COALESCE(start_at,created_at) DESC"));
  }
  if(p==="/api/public/units"&&request.method==="GET"){
    return json(await safePublicList(env,"SELECT code,name,unit_type,manager_name,email,status,data_json FROM units WHERE status!='Đã giải thể' ORDER BY name"));
  }
  ...
  if(p==="/api/lookup/certificate"&&request.method==="GET"){
    ...
    if(!code){
      ...
      },400,{'Access-Control-Allow-Origin':'*','Vary':'Origin','Cache-Control':'no-store'});
    }
    ...
    if(!row){
      ...
      },404,{'Access-Control-Allow-Origin':'*','Vary':'Origin','Cache-Control':'no-store'});
    }
...
const statusText={
  ...
  revoked:"Đã thu hồi"
    ...
    },200,{'Access-Control-Allow-Origin':'*','Vary':'Origin','Cache-Control':'no-store'});
  }
