...
if(fs.existsSync(firstPath)){const first=fs.readFileSync(firstPath,'utf8');if(publicText.includes(first.match(/Temporary password: (.+)/)?.[1]||'__never__')){console.error('TEMP PASSWORD LEAKED INTO PUBLIC');ok=false;}}
...
if(ok){console.log('SFN Production Master validation: OK');process.exit(0)}
process.exit(1);
