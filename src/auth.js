...
export async function getAuthUser(request,env){
  const token=parseCookies(request).sfn_session;
...
async function verifyPassword(env,email,password){
  ...
  const got=await pbkdf2(password,u.password_salt,u.password_iterations||150000);
...
export async function authRoute(request,env,url){
  ...
  if(p==="/api/auth/register"&&request.method==="POST"){
    ...
    const salt=newSalt(),hash=await pbkdf2(password,salt,150000);
    const res=await env.DB.prepare("INSERT INTO users(email,full_name,password_salt,password_hash,password_iterations,status,email_verified,must_change_password) VALUES(?,?,?,?,150000,'active',0,0)")
  ...
  if(p==="/api/auth/login"&&request.method==="POST"){
    ...
    if(portal==="member"&&!roleIds.some(r=>["member","volunteer","handler","unit_admin","communications","external_events","hr","office","network_secretary","system_admin","super_admin"].includes(r)))
  ...
  if(p==="/api/auth/change-password"&&request.method==="POST"){
    ...
    const salt=newSalt(),hash=await pbkdf2(next,salt,150000);
    await env.DB.batch([
      env.DB.prepare("UPDATE users SET password_salt=?,password_hash=?,password_iterations=150000,must_change_password=0,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(salt,hash,user.id),
  ...
  if(p==="/api/auth/reset"&&request.method==="POST"){
    ...
    const salt=newSalt(),hash=await pbkdf2(next,salt,150000);
    await env.DB.batch([
      env.DB.prepare("UPDATE users SET password_salt=?,password_hash=?,password_iterations=150000,must_change_password=0,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(salt,hash,row.user_id),
      env.DB.prepare("UPDATE auth_tokens SET used_at=CURRENT_TIMESTAMP WHERE id=?").bind(row.id),
  ...
  if(p==="/api/auth/google/callback"&&request.method==="GET"){
    ...
    if(st.portal==="member"&&!ids.some(r=>["member","volunteer","handler","unit_admin","communications","external_events","hr","office","network_secretary","system_admin","super_admin"].includes(r)))
  ...
  if(p==="/api/me/2fa/setup"&&request.method==="POST"){
    ...
    const issuer=encodeURIComponent("Sky First Network"),label=encodeURIComponent(user.email);
...
export async function createInvitedAccount(request,env,actor,body){
  ...
  let temp="SFN!"; for(let i=0;i<rnd.length;i++) temp+=chars[rnd[i]%chars.length];
  const salt=newSalt(),hash=await pbkdf2(temp,salt,150000);
  const res=await env.DB.prepare("INSERT INTO users(email,full_name,password_salt,password_hash,password_iterations,status,email_verified,must_change_password) VALUES(?,?,?,?,150000,'active',1,1)")
