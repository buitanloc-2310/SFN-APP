...
export async function pbkdf2(password, saltB64, iterations=150000){
...
export function sessionCookie(token, maxAge=604800){
  return `sfn_session=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${maxAge}`;
}
...
export async function ipHash(request){
  return sha256("sfn-ip:"+requestIp(request));
}
